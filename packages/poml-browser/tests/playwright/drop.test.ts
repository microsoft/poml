...
test.describe('parseUriList helper function', () => {
  test('parses valid URI list with CRLF', async ({ sidebarPage }) => {
    const result = await sidebarPage.evaluate(() => {
      // Access the parseUriList function indirectly through testing
      const uriList = 'http://example.com\r\nhttp://example.org\r\nhttp://example.net';

      // Simulate parseUriList behavior
      const urls = uriList
        .split(/\r\n|\n/)
        .map((l) => l.trim())
  ...
  test('parses URI list with LF fallback', async ({ sidebarPage }) => {
    const result = await sidebarPage.evaluate(() => {
      const uriList = 'http://example.com\nhttp://example.org\nhttp://example.net';
  ...
  test('filters out comment lines', async ({ sidebarPage }) => {
    const result = await sidebarPage.evaluate(() => {
  ...
  test('handles empty lines and whitespace', async ({ sidebarPage }) => {
    const result = await sidebarPage.evaluate(() => {
      const uriList = '  \r\nhttp://example.com\r\n\r\n  http://example.org  \r\n\r\n';
