 ...
 */
export async function cardFromPdf(
  file: string | File | Blob | ArrayBuffer,
  options?: CardFromPdfOptions,
): Promise<CardModel> {
  try {
    return (await _cardFromPdf(file, options)).card;
  } catch (error) {
 ...
 * The implementation of cardFromPdf
 */
export async function _cardFromPdfImpl(
  file: string | File | Blob | ArrayBuffer,
  options?: CardFromPdfOptions,
...
function getTextBoundingBoxes(pdfTextContent: any, viewport: any): BoundingBox[] {
  const boxes: BoundingBox[] = [];

  for (const item of (pdfTextContent as any).items) {
